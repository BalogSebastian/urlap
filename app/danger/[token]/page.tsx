import DangerUploadForm from "@/app/danger/[token]/DangerUploadForm";
...
export default async function DangerPage({
    ...
    return (
      <main className="min-h-screen w-full bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 max-w-md w-full p-8 text-center">
          <h1 className="text-2xl font-extrabold text-slate-900 mb-3">A link érvénytelen vagy lejárt</h1>
          <p className="text-sm text-slate-500">Kérjük, jelezze nekünk, hogy új meghívó linket küldhessünk.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen w-full bg-slate-50 py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
      <DangerUploadForm formType={info.formType} />
    </main>
  );
}
